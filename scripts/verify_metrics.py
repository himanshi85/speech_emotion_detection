...
def audit_model(key: str) -> tuple[bool, list[str], list[str]]:
    root = OUTPUTS_ROOT / key
    ok_notes: list[str] = []
    issues: list[str] = []
    ...
    for split in SPLITS:
        ...
        if _exists(rep):
            df = pd.read_csv(rep)
            n_emotions = len(df[df["emotion"].isin(
                ["neutral", "calm", "happy", "sad", "angry", "fearful", "disgust", "surprised"]
            )])
            if n_emotions != REPORT_EMOTIONS:
                issues.append(
                    f"{key}: {split} classification_report has {n_emotions} emotions, expected 8"
                )
...
def audit_combined(trained_keys: list[str]) -> tuple[list[str], list[str]]:
    ok_notes: list[str] = []
    issues: list[str] = []
    ...
    for rel in COMBINED_FILES:
        p = COMPARISON_DIR / rel
        if not _exists(p):
            issues.append(f"comparison: missing {rel}")

    for key in trained_keys:
        cm = COMPARISON_DIR / "confusion_matrices" / f"test_{key}.png"
        if not _exists(cm):
            issues.append(f"comparison: missing confusion_matrices/test_{key}.png")

    if _exists(COMPARISON_DIR / "all_models_results.csv"):
        df = pd.read_csv(COMPARISON_DIR / "all_models_results.csv")
...
def main() -> int:
    print("SER Metrics & Artifacts Audit")
    print("=" * 72)
    ...
    for key in ALL_MODEL_KEYS:
        ok, notes, issues = audit_model(key)
        if ok:
            trained.append(key)
        all_ok_notes.extend(notes)
        all_issues.extend(issues)

    comb_ok, comb_issues = audit_combined(trained)
    ...
    print("\nExpected combined (outputs/comparison/):")
    ...
    report_path = COMPARISON_DIR / "metrics_audit.txt"
    COMPARISON_DIR.mkdir(parents=True, exist_ok=True)
    lines = [f"Trained: {len(trained)}/{len(ALL_MODEL_KEYS)}", ""]
