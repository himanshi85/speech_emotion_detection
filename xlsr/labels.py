"""
Section 4 — Locked 8-class emotion label mapping for RAVDESS SER.

Do NOT change this mapping later. All future models in this research
comparison must use the same IDs.

The classification head must output exactly 8 logits.
"""
...
if TYPE_CHECKING:
    from xlsr.dataset_io import DatasetBundle

logger = logging.getLogger(__name__)

# Exactly 8 classes — locked for the entire research project
NUM_CLASSES = 8
...
def num_classifier_logits() -> int:
    """Classifier must output this many logits."""
    return NUM_CLASSES
...
def verify_labels_in_dataframe(
    ...
    # All 8 emotions should appear in each split for a valid SER experiment
...
def verify_emotion_classes(bundle: "DatasetBundle") -> LabelVerificationReport:
    ...
    # Unique labels across all data must be exactly 0..7
    all_labels = set()
...
def run_label_verification(
    ...
    """Load splits, verify locked labels, write report under outputs/."""
    # Local imports avoid circular dependency with dataset_io / constants.
