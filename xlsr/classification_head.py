"""
Section 9 — Simple SER classification head.

    Dropout(0.3) → Linear(hidden_size → 8)

Keep the head simple to evaluate the pretrained XLS-R representation.
No multi-layer MLP, no ASR/CTC head.
"""
...
from xlsr.model import DEFAULT_DROPOUT, Wav2Vec2XLSRForSER, build_xlsr_ser_model
...
def verify_classification_head(
    ...
    """Validate the SER head matches Section 9 requirements."""
    issues: List[str] = []
    checks: List[str] = []
    details: dict = {}
    ...
    expected_params = in_f * out_f + out_f  # weight + bias
    ...
    if not required.issubset(set(named.keys())):
        ...
        checks.append("PASS: modules are encoder + dropout + classifier only (+extras none)")

    # Extra children beyond the simple design?
    extras = set(named.keys()) - required
    details["extra_modules"] = sorted(extras)
    if extras:
        # Allow nothing else on this SER wrapper
    ...
    # --- Behavioral: dropout before linear ---
    model.train()
    torch.manual_seed(0)
    pooled = torch.randn(4, model.hidden_size)

    # Force dropout path: with p=0.3 in train mode outputs should usually differ
    with torch.no_grad():
        ...
        # In eval, dropout is identity — repeated calls match
    ...
    # Train-mode dropout should change values across two stochastic passes (very likely)
    if torch.allclose(y1, y2):
        # Rare but possible; try more draws
        differs = False
        model.train()
        with torch.no_grad():
