"""
Load facebook/wav2vec2-xls-r-300m directly from the Hugging Face Hub.

Uses HF_TOKEN from the project .env file when present (recommended for
authenticated Hub access / higher rate limits).
"""
...
# Project root: .../Speech_audio_emotion
...
def load_hf_token(env_path: Optional[Path] = None) -> Optional[str]:
    ...
    Returns None if no token is set (public Hub downloads may still work).
    """
...
def load_xlsr_feature_extractor(
    ...
    """Load the official feature extractor for Wav2Vec2-XLS-R-300M."""
    if token is None:
        token = load_hf_token()
    ...
    # XLS-R expects 16 kHz; enforce consistency with preprocessed RAVDESS audio
...
def load_xlsr_encoder(
    ...
    """
    Load the pretrained Wav2Vec2-XLS-R-300M encoder from Hugging Face.

    Uses Wav2Vec2Model (encoder only) — not an ASR/CTC head.
    Classification head will be added in a later plan section.
    """
...
def load_xlsr_config(
    ...
    """Load model config from the Hub (lightweight; no weights)."""
    if token is None:
        token = load_hf_token()
...
def load_xlsr_from_hub(
    ...
    """
    Convenience loader: config + feature extractor + (optional) encoder weights.

    Returns:
        (model_or_None, feature_extractor, config)
    """
    if model_name != MODEL_NAME:
        raise ValueError(
            f"This experiment must use exactly '{MODEL_NAME}'. Got: '{model_name}'"
