...
def save_predictions_csv(
    ...
    id_to_emotion: Dict[int, str] | None = None,
) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    mapping = id_to_emotion if id_to_emotion is not None else ID_TO_EMOTION
    rows = []
        ...
        rows.append(
            {
                ...
                "true_label": y_true[i],
                "true_emotion": mapping.get(y_true[i], str(y_true[i])),
                "predicted_label": y_pred[i],
                "predicted_emotion": mapping.get(y_pred[i], str(y_pred[i])),
